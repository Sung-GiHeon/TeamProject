...
<div class="inner" style="height:100%;">
	...
	<h3 >BOARD Modify</h3>
	<form id="registerForm" action="board_modify" method="post">
		<input type="hidden" name="supportNum" value="${board.supportNum}" />
		<input type="hidden" name="userNumcopy" value="${board.userNum}" />
		...
		<table>	
			...
			<tr>
				<td>작성자</td>
				<td><input style="width:100%;" type="text" name="writer"  placeholder="작성자" value="${board.writer}" disabled/></td>
			</tr>
			<tr>
				<td>내용</td>
				<td>
					<textarea style="width:100%;" cols=10 rows="10" name="content" placeholder="내용">${board.content}</textarea>
				</td>
			</tr>
			...
			<tr>
				<td>시작일</td>
				<td>
					<f:formatDate  pattern="yyyy-MM-dd" value="${board.regdate}" var="start_day"/>
					<input style="width:100%;" type="text" id="date_start" value="${start_day}" placeholder="당일부터 가능합니다." disabled /> 
				</td>
			</tr>
			<tr>
				<td>종료일</td>
				<td>
					<f:parseDate var="endString" value="${board.enddate}" pattern="yyyy-MM-dd" /> 
					<f:formatDate  pattern="yyyy-MM-dd" value="${endString}" var="end_day"/>
					<input style="width:100%;" type="text" id="date_end" name="enddate" value="${end_day}" placeholder="마감일을 정해주세요." required />
				</td>
			</tr>
...
<script>
	...
	$("#saveBtn").click(function(){
		...
		console.log("input 개수 : "+input_num.length);
